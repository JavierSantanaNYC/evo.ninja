...
interface NslookupQueryFuncParameters {
  endpoint: string;
}

export class NslookupQueryFunction extends ScriptFunction<NslookupQueryFuncParameters> {
  constructor(scripts: Scripts) {
    super(scripts);
  }

  name: string = "powershell_nslookup";
  ...
  onSuccess(
    agent: Agent,
    params: NslookupQueryFuncParameters,
